...
export const postBookUserTicket = createAsyncThunk('postBookUserTicket/apiUserTicket', async (obj, thunkAPI) => {
    const token = thunkAPI.getState().apiLoginLogout.token; //lấy token bên apiLoginLogout 
    const res = await axios.post(`http://localhost:8000/api/ticket/addNew`, obj, {
        headers: {
            'token': `${token}`
        }
    });
...
export const fetchListTicketUser = createAsyncThunk('fetchListTicketUser/apiUserTicket', async (id, thunkAPI) => {
    const token = thunkAPI.getState().apiLoginLogout.token; //lấy token bên apiLoginLogout 
    const res = await axios.get(`http://localhost:8000/api/ticket/getDetailWithUser?userId=${id}`, {
        headers: {
            'token': `${token}`
        }
    });
