...
function App() {
  ...
  const statusPostLogin = useSelector((state) => state.apiLoginLogout.statusPostLogin);
  const statusLogout = useSelector((state) => state.apiLoginLogout.statusLogout);
  const statusPostSignUp = useSelector((state) => state.apiSignup.statusPostSignUp);
  const statusUpdateUser = useSelector((state) => state.apiUpdateUser.statusUpdateUser);
  const statusBookUserTicket = useSelector((state) => state.apiUserTicket.statusBookUserTicket);
  ...
  useEffect(()=>{
    if(statusUpdateUser == "succeeded") toast.success("Đã thay đổi thông tin !");
  }, [statusUpdateUser]);

  useEffect(()=>{
    if(statusBookUserTicket == "succeeded") toast.success("Đặt vé thành công !");
